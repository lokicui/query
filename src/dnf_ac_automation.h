...
public:
    ...
    bool next(pageid_t *doc, std::vector<const pattern_t*> *match_patterns, pageid_t predoc)
    {
        // 根据AC自动机求交获得下一篇文档的docid
        predoc++;
        node_t *p(root_);
        pageid_t minid(-1);
again:
        do
        {
            for (next_iterator_type it = p->next_->begin(); it != p->next_->end(); ++it)
            {
               node_t *cur(it->second);
               pageid_t bound;
               if (cur->get()->seek_lower_bound(&bound, predoc) && bound == predoc)
               {
                   p = cur;
        ...
        // 能到这里来必然是没有找到predoc, predoc设置为minid+1，然后继续找
        predoc = minid + 1;
        minid = pageid_t(-1);
        p = p->fail_;
        if (p != root_)
