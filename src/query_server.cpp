        ...
        Closure<void>* done)
{
    ...
    bool accept = false;
    if (url.find("accept=1") != std::string::npos)
        accept = true;
    string kwds, kwds_decoded;
    const string start_sign = "kwds=";
    ...
    if (hitall && !queryterms.empty())
    {
        ...
        while ( cnt < statistics_threshold && intersector.next(&pageid, pageid))
        {
            ...
            if (docinfo)
            {
                ...
                uint32_t date = docinfo->time_ / (86400 * 7);
                if (date != 0)
                {
                    if (date_st.find(date) == date_st.end())
                        date_st[date] = 0;
                    date_st[date] ++;
                }
                if (accept && !docinfo->selected_)
                    continue;
                cnt ++;
                classid_st[docinfo->classid_] ++;
                selected_cnt += docinfo->selected_;
                tadopt_rate += docinfo->adoptrate_;
    ...
    Json::Value date_st_array;
    for (std::map<uint32_t, size_t>::const_iterator it = date_st.begin(); it != date_st.end(); ++ it)
    {
        date_st_array[StringFormat("%u", it->first)] = Json::UInt64(it->second);
    }

    Json::Value root;
    Json::Value statistics;
    statistics["dateinfo"] = date_st_array;
    statistics["classinfo"] = class_st_array;
    statistics["st_cnt"] = Json::UInt(cnt);
    statistics["selected_cnt"] = Json::UInt(selected_cnt);
    statistics["tadopt_rate"] = Json::UInt(tadopt_rate);
